...
///
/// - "classic": Load embedded classic theme
/// - "accessible": Load embedded accessible theme
/// - "*.toml": Load theme from file path
/// - Other: Error with available options
fn load_theme(name: &str) -> Result<ColorTheme> {
    // Try built-in themes first
    if let Ok(theme) = color_adapter::load_builtin_theme(name) {
        return Ok(theme);
    }
    ...
    if name.ends_with(".toml") {
        ...
        return ColorTheme::load_from_file(path)
            .with_context(|| format!("Failed to load theme from {}", path.display()));
    }

    // Unknown theme
    anyhow::bail!(
        "Unknown theme '{}'. Available: classic, accessible, or provide a .toml file path.",
        name
    )
}
...
fn read_menu_choice() -> Result<String> {
    ...
    io::stdin().read_line(&mut input).context("read menu choice")?;
...
fn read_line(prompt: &str) -> Result<String> {
    ...
    io::stdout().flush().context("flush stdout")?;
    let mut input = String::new();
    io::stdin().read_line(&mut input).context("read line")?;
    Ok(input.trim().to_string())
}
...
fn main() -> Result<()> {
    ...
    // Load theme early to fail fast if invalid
    let theme = load_theme(&args.theme)?;
    println!("Theme loaded: {}", theme.meta.name);

    let mode = loop {
        println!("Select game mode:");
        println!("1. Classic (frozen parity)");
        println!("2. Modern (isolated evolution)");
        let value = read_menu_choice()?;
        match value.as_str() {
    ...
    let save_slot = default_save_slot_path_for_mode(mode);
        ...
        match choice.as_str() {
            "1" => {
                ...
                println!(
                    ...
                    final_state.status,
                    final_state.clock.turn,
                    final_state.player.stats.hp,
                    final_state.player.stats.max_hp
                );
...
mod tests {
    ...
    fn questionnaire_creation_is_applicable_to_runtime_state() {
        ...
        let derived = derive_legacy_questionnaire_creation("Iria".to_string(), &answers);
