...
use palette::{FromColor, LinLuma, Srgb};
...
fn validate_theme(path: &Path) -> Result<()> {
    if path.is_dir() {
        ...
        println!(
            "
Summary: {} themes validated, {} failures",
            count, failures
        );
        if failures > 0 {
            std::process::exit(1);
...
fn validate_single_file(path: &Path) -> Result<()> {
    ...
    theme.validate_strict().map_err(|errors| {
...
Semantic Colors:"
    ...
    if let Some((fg, _)) = theme.resolve_reference("semantic.danger") {
        check("danger", fg, &mut failures);
    }
    if let Some((fg, _)) = theme.resolve_reference("semantic.success") {
        check("success", fg, &mut failures);
    }
    if let Some((fg, _)) = theme.resolve_reference("semantic.info") {
        check("info", fg, &mut failures);
    }
    if let Some((fg, _)) = theme.resolve_reference("semantic.warning") {
        check("warning", fg, &mut failures);
    }

    if failures > 0 {
        eprintln!(
            "
Accessibility check FAILED with {} contrast issues",
            failures
        );
        std::process::exit(1);
    } else {
...
fn contrast_ratio(fg: omega_core::color::HexColor, bg: omega_core::color::HexColor) -> f32 {
    ...
    let f_color =
        Srgb::new(fr as f32 / 255.0, fg_g as f32 / 255.0, fb as f32 / 255.0).into_linear();
    let b_color =
        Srgb::new(br as f32 / 255.0, bg_g as f32 / 255.0, bb as f32 / 255.0).into_linear();
...
fn preview_theme(path: &Path) -> Result<()> {
    ...
    let p = &theme.base;
    print_swatch("black", p.black);
    print!("  ");
    print_swatch("red", p.red);
    println!();
    print_swatch("green", p.green);
    print!("  ");
    print_swatch("yellow", p.yellow);
    println!();
    print_swatch("blue", p.blue);
    print!("  ");
    print_swatch("magenta", p.magenta);
    println!();
    print_swatch("cyan", p.cyan);
    print!("  ");
    print_swatch("white", p.white);
    println!();
    print_swatch("gray", p.gray);
    println!();

    println!("\nSemantic Samples:");
    if let Some((fg, _)) = theme.resolve_reference("semantic.danger") {
        print_swatch("danger", fg);
        println!();
    }
    if let Some((fg, _)) = theme.resolve_reference("semantic.success") {
        print_swatch("success", fg);
        println!();
    }
    if let Some((fg, _)) = theme.resolve_reference("semantic.info") {
        print_swatch("info", fg);
        println!();
    }
