...
fn run_base_extractor() -> Result<()> {
    let status = Command::new("cargo")
        ...
        .status()
        .context("run legacy_mechanics_extract")?;
    if !status.success() {
        bail!("legacy_mechanics_extract failed");
    }
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    let src_json = Path::new("target/legacy-mechanics-ledger.json");
    let src_md = Path::new("target/legacy-mechanics-ledger.md");
    let dst_json = contracts_dir().join("legacy-mechanics-ledger.json");
    let dst_md = contracts_dir().join("legacy-mechanics-ledger.md");
    ...
    if total == 0 {
        bail!("legacy exhaustive ledger is empty");
    }
    println!("legacy mechanics exhaustive extract: total={} out={}", total, dst_json.display());
    Ok(())
}
