...
use omega_core::simulation::{
    ...
    displacement::{apply_explosive_displacement, check_explosion_trigger},
    environmental::{apply_fire_spread_bias, apply_gas_rise, apply_liquid_flow},
    grid::CaGrid,
...
pub fn particle_physics_system(
    ...
    for (mut particle, mut transform) in query.iter_mut() {
        ...
        if grid.in_bounds(x as isize, y as isize) {
            ...
            if matches!(cell.solid, Some(omega_core::simulation::Solid::Stone))
                && particle.logical_pos.z < 1.0
            {
                // Simplified bounce
    ...
    for mut particle in query.iter_mut() {
        ...
        if wind.in_bounds(x as isize, y as isize) {
            ...
            if vector.strength > 0 {
                let force = Vec3::new(vector.dx as f32, vector.dy as f32, 0.0)
                    * (vector.strength as f32 / 255.0)
                    * 10.0;
...
pub fn projectile_movement_system(
    ...
    for (mut projectile, mut transform) in query.iter_mut() {
        ...
        transform.translation.y =
            projectile.logical_pos.y + (projectile.logical_pos.z * VISUAL_Z_SCALE);
...
pub fn projectile_collision_system(
    ...
    for (entity, projectile) in projectiles.iter_mut() {
        ...
        if projectile.logical_pos.z <= 0.1 {
            ...
            if grid.in_bounds(x as isize, y as isize) {
                ...
                if let Some(impact_cell) = projectile.element_impact {
                    ...
                    if target.gas.is_none() {
                        target.gas = impact_cell.gas;
                    }
                    ...
                    if target.pressure > 200 {
                        reaction_logs.send(ReactionLog {
                            description: format!(
                                "Explosive displacement at ({},{}) radius:3",
                                x, y
                            ),
                        });
        ...
        for (_m_entity, m_pos, _m_stats) in monsters.iter() {
            ...
            if dist_sq < 0.25 && projectile.logical_pos.z < 1.5 {
                // Flying check height
...
fn merge_displacement_part(acc: &mut Cell, part: Cell) {
    if let Some(gas) = part.gas {
        acc.gas = Some(gas);
    }
    if part.solid.is_some() && acc.solid.is_none() {
        acc.solid = part.solid;
    }
    acc.heat = acc.heat.saturating_add(part.heat);
    acc.wet = acc.wet.saturating_add(part.wet);
    acc.pressure = acc.pressure.saturating_add(part.pressure);
}

pub fn update_ca_cells(mut grid: ResMut<CaGrid>, wind: Res<WindGrid>, tick: Res<SimulationTick>) {
    let (w, h) = (grid.width(), grid.height());
    let is_reclaim_tick = tick.0.is_multiple_of(60);
    let mut displaced_parts: BTreeMap<(usize, usize), Cell> = BTreeMap::new();
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            // 2. Wind effects
            let (after_wind, displaced) = apply_wind(&grid, &wind, x, y, &next);
            next = after_wind;
            if let Some((tx, ty, part)) = displaced {
                let entry = displaced_parts.entry((tx, ty)).or_default();
                merge_displacement_part(entry, part);
            }
    ...
    for ((tx, ty), part) in displaced_parts {
        let mut target = *grid.get(tx, ty);
        merge_displacement_part(&mut target, part);
        grid.set(tx, ty, target);
    }
}
...
mod tests {
    use super::*;
    use omega_core::simulation::state::{Gas, Solid};

    #[test]
    fn merge_displacement_part_accumulates_state() {
        let mut acc = Cell::default();
        let part_a = Cell { gas: Some(Gas::Smoke), heat: 20, pressure: 10, ..Cell::default() };
        let part_b = Cell { solid: Some(Solid::Ash), heat: 30, pressure: 5, ..Cell::default() };

        merge_displacement_part(&mut acc, part_a);
        merge_displacement_part(&mut acc, part_b);

        assert_eq!(acc.gas, Some(Gas::Smoke));
        assert_eq!(acc.solid, Some(Solid::Ash));
        assert_eq!(acc.heat, 50);
        assert_eq!(acc.pressure, 15);
    }
}
